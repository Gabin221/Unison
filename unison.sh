...
DIRS=("exercism" "Documents" "bin" "Images" ".local/share/applications")
...
check_disk() {
    ...
    if [ -d "${DISK_MOUNT_PATH}/unison" ]; then
        ...
        for DIR in "${DIRS[@]}"; do
            ...
            unison ~/$(basename ${DIR}) /media/<your name>/DISQUE\ ESSB/unison/$(basename ${DIR}) -auto -batch -confirmbigdel=false -prefer newer -ignore 'Path env' -perms 0
        done
    else
